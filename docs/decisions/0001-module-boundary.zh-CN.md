# ADR-001：保持 codec-websocket 为独立 Gnalloy 模块
...
`codec-websocket` 作为 `gnalloy.org/codec-websocket` 发布，并拥有该职责：Gnalloy WebSocket frame、handshake、Pipeline Handler 与 permessage-deflate 支持。
